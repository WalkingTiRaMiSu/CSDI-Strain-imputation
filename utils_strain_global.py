...
# Global masking CSDI train/evaluate/plot utilities.
...
def train(
    ...
    print("\n" + "=" * 90)
    print("[TRAIN SETUP] CSDI strain GLOBAL block-missing training")
    ...
    for epoch_no in range(epochs):
        ...
        if did_validation:
            ...
            with torch.no_grad():
                with tqdm(
                    valid_loader,
                    desc=f"Valid {epoch:03d}/{epochs:03d}",
                    mininterval=1.0,
                    maxinterval=10.0,
                    dynamic_ncols=True,
                ) as it:
        ...
        print(
            f"[Epoch {epoch:03d}/{epochs:03d}] END | "
            f"train_loss={train_loss:.6f} | {valid_msg} | "
            f"batches={batch_count} | time={elapsed:.1f}s"
        )
...
def plot_reconstruction(
    ...
    if missing_start is not None and missing_end is not None:
        plt.axvspan(t[missing_start], t[missing_end - 1], alpha=0.18, label="Global removed STr block")

    if xta_peak_idx is not None:
        plt.axvline(t[xta_peak_idx], linestyle="--", linewidth=1.0, label="XTa abs peak")

    if zoom and missing_start is not None and missing_end is not None:
        miss_len = max(1, missing_end - missing_start)
        pad = max(int(2.0 * miss_len), int(0.10 * (active_end - active_start)))
        left = max(0, missing_start - pad)
        right = min(len(t) - 1, missing_end + pad)
...
def evaluate_and_plot(
    ...
    os.makedirs(foldername, exist_ok=True)
    plot_dir = os.path.join(foldername, "plots_reconstruction_global")
    ...
    print("\n" + "=" * 90)
    print("[EVAL SETUP] CSDI global sampling/evaluation")
    ...
    with torch.no_grad():
        with tqdm(test_loader, mininterval=5.0, maxinterval=50.0, dynamic_ncols=True) as it:
            for test_batch in it:
                ...
                # samples: (B, nsample, K, L) -> (B, nsample, L, K)
    ...
    for scenario_key, obj in collected.items():
        ...
        missing_start = int(obj["missing_start"])
        missing_end = int(obj["missing_end"])
        global_missing_mask = np.zeros(n, dtype=bool)
        global_missing_mask[missing_start:missing_end] = True
        ...
        missing_len = int(missing_end - missing_start)
        ...
        metrics_rows.append({
            ...
            "missing_start_idx": missing_start,
            "missing_end_idx": missing_end,
            "missing_start_time_s": float(t[missing_start]),
            "missing_end_time_s": float(t[missing_end - 1]),
        ...
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["idx", "time_s", "true_strain", "pred_median", "pred_min", "pred_max", "num_values", "is_global_missing"])
            for i in range(n):
        ...
        plot_reconstruction(
            ...
            active_start, active_end, int(obj["xta_peak_idx"]), missing_start, missing_end,
            os.path.join(plot_dir, f"{base}_full.png"),
            zoom=False,
        )
        plot_reconstruction(
            t, true_str, pred_median, pred_min, pred_max,
            active_start, active_end, int(obj["xta_peak_idx"]), missing_start, missing_end,
            os.path.join(plot_dir, f"{base}_missing_zoom.png"),
            zoom=True,
        )
        plot_masked_input(
            t, true_str, global_missing_mask,
            active_start, active_end, int(obj["xta_peak_idx"]), missing_start, missing_end,
            os.path.join(plot_dir, f"{base}_masked_input_zoom.png"),
            zoom=True,
        )
    ...
    metrics_path = os.path.join(foldername, "reconstruction_metrics_global.csv")
    with open(metrics_path, "w", newline="", encoding="utf-8") as f:
        fieldnames = [
            "scenario_key", "event_no", "position", "missing_ratio",
            ...
            "num_reconstructed_points", "missing_points",
            "reconstructed_ratio_of_full", "reconstructed_ratio_of_active", "rmse", "mae",
        ]
    ...
    pk_path = os.path.join(foldername, f"generated_outputs_global_nsample{nsample}.pk")
    with open(pk_path, "wb") as f:
        pickle.dump(all_window_outputs, f)

    print(f"[RESULT] overall global RMSE: {overall_rmse:.6f}")
    print(f"[RESULT] overall global MAE : {overall_mae:.6f}")
